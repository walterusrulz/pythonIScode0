...
class Astar(Search):
    ...
    def doSearch(self):
        ...
        open_nodes.put((root_node.eval, root_node.id, root_node))  # a triple
        self.nGenerated += 1
            ...
            if current_node.hashcode not in visitedNodes:
                ...
                if current_node.state.isFinal():  # first we check if the state is final
                    ...
                    for each_action in possibleActions:
                        ...
                        node_generated.eval = node_generated.cost + node_generated.heuristic  # eval function Astar
                        open_nodes.put((node_generated.eval, node_generated.id, node_generated))  # a triple
                        # first priority is
